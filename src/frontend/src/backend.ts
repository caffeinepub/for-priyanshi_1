...
export class Backend implements backendInterface {
    ...
    async getResponse(): Promise<Response | null> {
        if (this.processError) {
            try {
                const result = await this.actor.getResponse();
    ...
    async submitResponse(arg0: string): Promise<void> {
        if (this.processError) {
            try {
...
function from_candid_opt_n1(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_Response]): Response | null {
